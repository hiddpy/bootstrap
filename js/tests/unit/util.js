$(function () {
  ...
  QUnit.test('Util.getTransitionDurationFromElement should properly get parent transition', function (assert) {
    assert.expect(1)
    var $parent = $('<div style="transition-duration: 5s;"></div>')
    var $child = $('<div></div>')
    $('#qunit-fixture').append($parent.append($child))

    assert.strictEqual(Util.getTransitionDurationFromElement($child[0]), 5000)
  })
